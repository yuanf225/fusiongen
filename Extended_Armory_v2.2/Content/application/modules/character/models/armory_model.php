...
class Armory_model extends CI_Model
{
	...
	public function getTalents($spec)
	{
		...
		$statements['trinity_bfa'] = "SELECT `talent_id` FROM `character_talent` WHERE `guid` = ? AND `talentgroup` = ? ORDER BY `talent_id` DESC;";
		$statements['arkcore'] 		= 
		$statements['azerothcore']  = "SELECT `spell` FROM `character_talent` WHERE `guid` = ? AND `specMask` = ? ORDER BY `spell` DESC;";
	...
	public function getTalentSpecsInfo()
	{
		...
		$statements['arkcore'] 		=
		$statements['azerothcore']  = "SELECT `talentGroupsCount` AS speccount, `activeTalentGroup` AS activespec FROM `characters` WHERE `guid` = ? LIMIT 1;";
