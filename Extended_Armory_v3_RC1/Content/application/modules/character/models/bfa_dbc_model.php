...
class bfa_dbc_model extends CI_Model
{
	...
	{
		$this->db->select('*')->from($this->getTablePrefix().'_talent')->where(array('SpecID' => $tab))->order_by('id', 'ASC');
		$query = $this->db->get();
	...
	public function getAchievementInfo($id)
	{
		$query = $this->db->query("	SELECT 
										...
										`".$this->getTablePrefix()."_achievement`.`iconFileID`,  
										`".$this->getTablePrefix()."_spellicons`.`iconname` 
									FROM `".$this->getTablePrefix()."_achievement` 
									LEFT JOIN `".$this->getTablePrefix()."_spellicons` ON `".$this->getTablePrefix()."_spellicons`.`id` = `".$this->getTablePrefix()."_achievement`.`iconFileID` 
